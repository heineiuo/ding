...
function createServer(argv) {
  ...
  let rootConfigs = require(
    path.resolve(
      process.cwd(), 
      argv.config || './webpack.config.js'
    )
  )(process.env)
  ...
  if (targets.length === 0) {
    return console.log('No packages found.')
  }
  ...
  if (argv.hot) app.use(webpackHotMiddleware(compiler, {
    path: targets[0].output.publicPath
  }))
...
module.exports = module.exports.default = createServer