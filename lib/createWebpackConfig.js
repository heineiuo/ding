...
const createWebpackConfig = (configFile) => {
  ...
  const outputPath = path.resolve(context, outputDir) + '/'
	...
	// const umd = path.resolve(`/${packageJSON.name}@${packageJSON.version}/`, outputDir);
	const umd = `/${packageJSON.name}@${packageJSON.version}/umd`;
  const publicPath = `${__DEV__ ? devUnpkgOrigin : unpkgOrigin}` + umd + '/'
  const entryName = path.basename(packageJSON.name)

  console.log(`babel-loader exclude ${webpackLoaderExclude(configFile.modulesNeedBabel)}`)
  const config = {
    context,
    devtool: false,
    node: {
